...
const COUNCIL_VICE_ROLE = 'vice_presidente';
const UNDERLYING_COUNCIL_ROLE = 'presidente';
...
export async function setCouncilViceAccess(env, username, enabled, actor = '') {
  ...
  const normalized = normalizeUsername(username);
  if (!normalized) throw new Error('Usuário inválido para função de Vice-Presidente do Conselho.');
...
export async function decorateCouncilViceUser(env, user) {
  ...
  if (String(user.councilRole || '') !== UNDERLYING_COUNCIL_ROLE) {
    return { ...user, councilVicePresident: false };
  }
  const enabled = await councilViceAccessFor(env, user.username);
  if (!enabled) return { ...user, councilVicePresident: false };
  const asPresident = env?.COUNCIL_VICE_AS_PRESIDENT === true || String(env?.COUNCIL_VICE_AS_PRESIDENT || '').toLowerCase() === 'true';
  return {
    ...user,
    councilRole: asPresident ? UNDERLYING_COUNCIL_ROLE : COUNCIL_VICE_ROLE,
    councilVicePresident: true,
...
export async function decorateCouncilViceUsers(env, users) {
  const output = [];
  for (const user of Array.isArray(users) ? users : []) output.push(await decorateCouncilViceUser(env, user));
  return output;
}

export function councilViceRoleName() {
  return COUNCIL_VICE_ROLE;
}

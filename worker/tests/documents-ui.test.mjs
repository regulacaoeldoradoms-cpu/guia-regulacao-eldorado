...
test('Central usa somente Worker para Drive e delega persistência documental ao cache criptografado', () => {
  ...
  assert.match(html, /id="documentsAccessChip"[^>]*hidden/);
  assert.match(client, /els\.accessChip\.textContent = drive\.writeEnabled === true \? 'Acesso completo' : 'Somente leitura'/);
...
test('Fase 7E pré-carrega somente a raiz após login e evita duplicar a chamada enquanto o warmup termina', () => {
  ...
  assert.doesNotMatch(performanceClient, /schedulePriorityDocumentFilesWarm|prefetchPriorityDocumentFiles|PortalDocumentCache/);
  ...
  assert.doesNotMatch(worker, /priorityFolders|Consulta \[2026\]|Exames \[2026\]/i);
  ...
  assert.match(client, /getDocumentWarmPayload[\s\S]*timeoutMs: 6000/);
  ...
  assert.match(client, /source:\s*'cache'/);
  assert.match(client, /cache_state:\s*'hit'/);
  assert.doesNotMatch(client, /warmedPriorityFolder|portal:documents-warm-updated/);
...
test('pesquisa avançada expõe filtros suportados sem simular criptografia ou aprovações', () => {
  const html = read('documentos/index.html');
  const client = read('js/documents.js');
  const drive = read('worker/document-drive.js');
  const router = read('worker/documents-router.js');
  ...
  assert.match(html, /id="documentsAdvancedSearchButton"/);
  assert.match(html, /id="documentsAdvancedSearchDialog"/);
  assert.match(html, /id="documentsAdvancedType"/);
  assert.match(html, /id="documentsAdvancedOwner"/);
  assert.match(html, /id="documentsAdvancedWords"/);
  assert.match(html, /id="documentsAdvancedItemName"/);
  assert.match(html, /id="documentsAdvancedLocation"/);
  assert.match(html, /id="documentsAdvancedStarred"/);
  assert.match(html, /id="documentsAdvancedTrashed"/);
  assert.match(html, /id="documentsAdvancedModified"/);
  assert.match(html, /id="documentsAdvancedSharedWith"/);
  assert.match(html, /Criptografia e fluxos de aprovação não possuem filtro direto/);

  assert.match(css, /\.documents-advanced-search-dialog/);
  assert.match(css, /\.documents-advanced-search-grid/);
  assert.match(css, /\.documents-advanced-search-trigger\.active/);

  assert.match(client, /function defaultAdvancedSearchFilters\(\)/);
  assert.match(client, /function advancedSearchHasCriteria\(/);
  assert.match(client, /function readAdvancedSearchFilters\(\)/);
  assert.match(client, /function openAdvancedSearch\(\)/);
  assert.match(client, /filters:\s*state\.searchFilters/);
  assert.match(client, /titleOnly:\s*state\.searchTitleOnly/);
  assert.match(client, /location:\s*els\.advancedLocation\?\.value/);
  assert.match(client, /parentRef:\s*els\.advancedLocation\?\.value === 'current' \? currentParentRef\(\) : ''/);
  assert.match(client, /els\.advancedSearchForm\?\.addEventListener\('submit'/);

  assert.match(router, /filters:\s*\{/);
  assert.match(router, /ownerEmail:\s*String\(filters\.ownerEmail/);
  assert.match(router, /modifiedAfter:\s*String\(filters\.modifiedAfter/);
  assert.match(drive, /async function buildDriveSearchQuery\(/);
  assert.match(drive, /mimeType =/);
  assert.match(drive, /'me' in owners/);
  assert.match(drive, /sharedWithMe = true/);
  assert.match(drive, /starred = true/);
  assert.match(drive, /modifiedTime >=/);
  assert.match(drive, /modifiedTime <=/);
  assert.match(drive, /in readers/);
  assert.match(drive, /in writers/);
  assert.match(drive, /in parents/);
  assert.doesNotMatch(client, /approvalId|canStartApproval|contentRestrictions/);
});

test('ordem cronológica alterna recente/antigo e permite restaurar a ordem original', () => {
  const html = read('documentos/index.html');
  const client = read('js/documents.js');
  const drive = read('worker/document-drive.js');
  const router = read('worker/documents-router.js');
  ...
  assert.match(html, /id="documentsChronologicalOrderButton"/);
  assert.match(html, /id="documentsResetOrderButton"[^>]*disabled/);
  assert.match(html, />\s*Ordem cronológica\s*</);
  assert.match(html, />\s*Redefinir ordem\s*</);
  assert.match(css, /\.documents-order-trigger/);
  assert.match(css, /\.documents-order-reset:disabled/);

  assert.match(client, /listSortOrder:\s*'original'/);
  assert.match(client, /function normalizeListSortOrder\(/);
  assert.match(client, /modified_desc/);
  assert.match(client, /modified_asc/);
  assert.match(client, /Mais recentes ↓/);
  assert.match(client, /Mais antigos ↑/);
  assert.match(client, /setListSortOrder\('original'\)/);
  assert.match(client, /sortOrder:\s*state\.listSortOrder/);
  assert.match(client, /state\.listSortOrder === 'original' && snapshot/);
  assert.match(client, /state\.listSortOrder === 'original'[\s\S]*!state\.searchMode/);

  assert.equal((router.match(/sortOrder:\s*String\(body\.sortOrder \|\| ''\)/g) || []).length >= 2, true);
  assert.match(drive, /function normalizeDriveListSortOrder\(/);
  assert.match(drive, /'modified_desc', 'modified_asc'/);
  assert.match(drive, /modifiedTime desc,name_natural/);
  assert.match(drive, /modifiedTime,name_natural/);
  assert.match(drive, /if \(orderBy\) url\.searchParams\.set\('orderBy', orderBy\)/);
  assert.doesNotMatch(drive, /url\.searchParams\.set\('orderBy',\s*input\.sortOrder/);
});
...
test('Fase 7E acelera navegação do Drive com raiz aquecida e sem preload de pastas especiais', () => {
  ...
  assert.match(client, /function normalizedSearchText/);
  assert.match(client, /snapshot\.items\.filter/);
  assert.equal((client.match(/pageSize:\s*20/g) || []).length >= 3, true);
  assert.doesNotMatch(client, /pageSize:\s*append \? 80 : 40/);
  assert.match(drive, /clampInteger\(input\.pageSize, 20, 20, 100\)/);
  assert.match(client, /drive_token_ms/);
  assert.match(client, /drive_api_ms/);
  assert.match(client, /drive_map_ms/);
  assert.match(client, /build_ms/);
  assert.match(client, /drive_start_ms/);
  assert.match(client, /drive_upload_ms/);
  assert.doesNotMatch(client, /localStorage[^\n]*folderSnapshot|sessionStorage[^\n]*folderSnapshot|indexedDB[^\n]*folderSnapshot/);
  assert.doesNotMatch(client, /warmedPriorityFolder|applyWarmedFolderSnapshot|portal:documents-warm-updated/);

  assert.match(drive, /const concurrency = Math\.min\(16, source\.length\)/);
  assert.match(drive, /cachedFileRefKey/);
  assert.match(drive, /cachedDriveCacheHmacKey/);
  assert.match(drive, /effectiveMime === PDF_MIME[\s\S]*stableDriveCacheKey/);
  const listBlock = drive.slice(
    drive.indexOf('export async function listDriveFolder'),
    drive.indexOf('export async function searchDrive')
  );
  const searchBlock = drive.slice(
    drive.indexOf('export async function searchDrive'),
    drive.indexOf('function normalizeDriveVersion')
  );
  assert.match(drive, /function driveListOrderBy\(/);
  assert.match(drive, /return search \? '' : 'folder,name_natural'/);
  assert.match(listBlock, /const orderBy = driveListOrderBy\(input\.sortOrder\)/);
  assert.match(searchBlock, /driveListOrderBy\(input\.sortOrder, \{ search: true \}\)/);

  for (const source of [observability, server]) {
    for (const metric of [
      'drive_token_ms', 'drive_api_ms', 'drive_map_ms',
      'build_ms', 'drive_start_ms', 'drive_upload_ms'
    ]) {
      assert.match(source, new RegExp(metric));
    }
    assert.doesNotMatch(source, /search_query|query_text|file_name|filename|fileId|drive_id|patient_name|cpf|cns/i);
  }
});
...
test('modo escuro mantém ferramentas do Titon transparentes com line-art clara', () => {
  const css = read('css/documents.css');
  const html = read('documentos/index.html');

  assert.match(css, /html\[data-portal-theme="dark"\] \.documents-editor-toolbar--compact \.documents-editor-tool:not\(\.documents-art-button\),[\s\S]*background-color:\s*transparent\s*!important;[\s\S]*border-color:\s*transparent\s*!important;[\s\S]*color:\s*#f5fbff\s*!important;[\s\S]*box-shadow:\s*none\s*!important/);
  assert.match(css, /html\[data-portal-theme="dark"\] \.documents-editor-toolbar--compact \.documents-tool-icon\s*\{[\s\S]*filter:\s*brightness\(0\) invert\(1\)/);
  assert.match(css, /html\[data-portal-theme="dark"\] \.documents-editor-side-rail \.documents-rail-tool svg\s*\{[\s\S]*fill:\s*none;[\s\S]*stroke:\s*currentColor/);
  assert.match(css, /\.documents-editor-tool:not\(\.documents-art-button\):hover:not\(:disabled\)[\s\S]*background-color:\s*transparent\s*!important/);
  assert.match(css, /\.documents-editor-tool:not\(\.documents-art-button\)\.active[\s\S]*background-color:\s*transparent\s*!important/);
  assert.match(css, /\.documents-editor-tool:not\(\.documents-art-button\):disabled[\s\S]*opacity:\s*\.46/);
  assert.doesNotMatch(css, /background-color:\s*#d8e9f4\s*!important/);
  assert.match(html, /documents\.css\?v=20260924-dark-final-1/);
});

test('modo escuro usa X branco transparente no cabeçalho do visualizador Titon', () => {
  const css = read('css/documents.css');
  const html = read('documentos/index.html');
  ...
  assert.match(css, /html\[data-portal-theme="dark"\] \.documents-viewer-head \.documents-close-viewer\.documents-art-close\s*\{[\s\S]*background:\s*transparent url\('\/assets\/editor-pdf-buttons\/fechar-dark\.svg\?v=20260924-1'\) center \/ 30px 30px no-repeat !important/);
  assert.ok(fs.existsSync(path.join(root, 'assets/editor-pdf-buttons/fechar-dark.svg')));
  assert.match(html, /documents\.css\?v=20260924-dark-final-1/);
});

test('modo escuro usa line-art transparente na navegação e preserva Salvar e Imprimir azuis', () => {
  const css = read('css/documents.css');
  const html = read('documentos/index.html');

  assert.match(css, /html\[data-portal-theme="dark"\] \.documents-pdf-toolbar \.documents-art-zoom-minus\s*\{[\s\S]*zoom-menos-dark\.svg\?v=20260924-1/);
  assert.match(css, /html\[data-portal-theme="dark"\] \.documents-pdf-toolbar \.documents-art-zoom-plus\s*\{[\s\S]*zoom-mais-dark\.svg\?v=20260924-1/);
  assert.match(css, /html\[data-portal-theme="dark"\] \.documents-pdf-toolbar \.documents-art-fit-width\s*\{[\s\S]*ajustar-largura-dark\.svg\?v=20260924-1/);
  assert.match(css, /html\[data-portal-theme="dark"\] \.documents-editor-toolbar--compact \.documents-art-grid\s*\{[\s\S]*grade-dark\.svg\?v=20260924-1/);
  assert.match(css, /html\[data-portal-theme="dark"\] \.documents-pdf-toolbar #pdfZoomResetButton\s*\{[\s\S]*background:\s*transparent\s*!important;[\s\S]*color:\s*#ffffff\s*!important/);
  assert.match(css, /html\[data-portal-theme="dark"\] \.documents-pdf-toolbar \.documents-art-export\s*\{[\s\S]*salvar-pdf\.svg\?v=20260916-2/);
  assert.match(css, /html\[data-portal-theme="dark"\] \.documents-pdf-toolbar \.documents-art-print\s*\{[\s\S]*imprimir-normal\.svg\?v=20260916-2/);

  for (const asset of [
    'zoom-menos-dark.svg',
    'zoom-mais-dark.svg',
    'ajustar-largura-dark.svg',
    'grade-dark.svg'
  ]) {
    assert.ok(fs.existsSync(path.join(root, 'assets/editor-pdf-buttons', asset)), asset);
  }
...
test('visualizador fora da edição mostra Salvar PDF e Imprimir ao lado de Ajustar largura', () => {
  ...
  const fit = html.indexOf('id="pdfFitWidthButton"');
  const save = html.indexOf('id="pdfSaveButton"');
  const print = html.indexOf('id="pdfPrintButton"');
  const count = html.indexOf('id="pdfPageCountLabel"');
  assert.ok(fit >= 0 && fit < save && save < print && print < count);

  assert.match(html, /id="pdfSaveButton"[^>]*documents-art-export[^>]*aria-label="Salvar PDF no dispositivo"/);
  assert.match(html, /id="pdfPrintButton"[^>]*documents-art-print[^>]*aria-label="Imprimir PDF"/);
  assert.match(client, /pdfSave:\s*document\.getElementById\('pdfSaveButton'\)/);
  assert.match(client, /pdfPrint:\s*document\.getElementById\('pdfPrintButton'\)/);
  assert.match(client, /async function saveViewedPdfLocal\(\)/);
  assert.match(client, /async function printViewedPdfLocal\(\)/);
  assert.match(client, /saveViewedPdfLocal[\s\S]*editablePdfBlob\(context\.item\)/);
  assert.match(client, /printViewedPdfLocal[\s\S]*editablePdfBlob\(context\.item\)/);
  assert.match(client, /printViewedPdfLocal[\s\S]*renderPdfBlobForPrint\(blob, frame\)/);
  assert.match(client, /els\.pdfSave\?\.addEventListener\('click'/);
  assert.match(client, /els\.pdfPrint\?\.addEventListener\('click'/);
  assert.match(client, /function syncViewerLocalActions\(\)/);
  assert.match(client, /control\.hidden = !visible/);
  assert.match(client, /setEditorSurfaceMode[\s\S]*syncViewerLocalActions\(\)/);
  assert.match(css, /\.documents-viewer-local-action\[hidden\][\s\S]*display:\s*none\s*!important/);
  assert.match(css, /\.documents-pdf-toolbar\s*\{[\s\S]*overflow-x:\s*auto/);
...
test('lista oferece salvar e imprimir PDF diretamente no hover sem abrir o Titon', () => {
  const client = read('js/documents.js');
  const css = read('css/documents.css');
  const html = read('documentos/index.html');

  assert.match(client, /data-list-save-index=/);
  assert.match(client, /data-list-print-index=/);
  assert.match(client, /async function saveListPdfLocal\(item, button\)/);
  assert.match(client, /async function printListPdfLocal\(item, button\)/);
  assert.match(client, /saveListPdfLocal[\s\S]*editablePdfBlob\(item\)[\s\S]*link\.download = localViewedPdfName\(item\)/);
  assert.match(client, /printListPdfLocal[\s\S]*editablePdfBlob\(item\)[\s\S]*ensurePrintFrame\(\)[\s\S]*renderPdfBlobForPrint\(blob, frame\)[\s\S]*printWindow\.print\(\)/);
  assert.match(client, /els\.list\.addEventListener\('click',[\s\S]*data-list-save-index[\s\S]*data-list-print-index/);

  const saveStart = client.indexOf('  async function saveListPdfLocal(');
  const printStart = client.indexOf('  async function printListPdfLocal(', saveStart);
  const quickEnd = client.indexOf('  async function finalPdfBlobForSession(', printStart);
  const quickBlock = client.slice(saveStart, quickEnd);
  assert.doesNotMatch(quickBlock, /openPdf\(|state\.pdfItem\s*=/);

  assert.match(css, /\.documents-item-quick-actions\s*\{[\s\S]*opacity:\s*0;[\s\S]*pointer-events:\s*none/);
...
test('lista renomeia PDF por segundo clique lento no nome sem substituir duplo clique de abertura', () => {
  ...
  assert.match(client, /function canWriteDocument\(item\)/);
  assert.match(client, /function canSyncDocuments\(\)\s*\{\s*return canWriteDocument\(state\.pdfItem\);/s);
  assert.match(client, /function beginListPdfRename\(index\)[\s\S]*if \(!canWriteDocument\(item\)\)/);
  assert.match(client, /async function commitListPdfRename\(\)[\s\S]*if \(!canWriteDocument\(previous\)\)/);
...
test('zoom do Titon mantém porcentagem em tempo real legível sobre fundo claro', () => {
  ...
  assert.match(css, /#pdfZoomResetButton #pdfZoomLabel\s*\{[\s\S]*?color:\s*var\(--documents-ui-text,\s*#111827\)\s*!important;[\s\S]*?opacity:\s*1\s*!important;/);

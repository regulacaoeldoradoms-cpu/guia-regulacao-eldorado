...
async function handleAdmin(request, env, url, origin) {
  ...
  if (!resetRoute && request.method === 'PATCH') {
    ...
    const bump = role !== target.role || active !== target.active ? 1 : 0;
    await env.AUTH_DB.prepare(`UPDATE auth_users
      SET name = ?, job_title = ?, role = ?, active = ?, session_version = session_version + ?, updated_at = CURRENT_TIMESTAMP
      WHERE username = ?`)
      .bind(name, jobTitle, role, active ? 1 : 0, bump, targetUsername).run();

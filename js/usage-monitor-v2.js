...
(async () => {
  ...
  if (!currentUser || !['admin', 'coordenacao'].includes(currentUser.role) || !endpoint) return;

  const state = { doctors: [], selected: '', period: 'day', timer: null };
  document.getElementById('portalUserName').textContent = currentUser.name || currentUser.username;
  document.getElementById('portalUserRole').textContent = currentUser.role === 'admin' ? 'Desenvolvedor · acesso técnico' : 'Coordenação · monitoramento';
  ...
  function localDateKey(date = new Date()) {
    const parts = new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Campo_Grande', year: 'numeric', month: '2-digit', day: '2-digit' })
  ...
  function formatDateTime(value) {
    const parsed = parseServerDate(value);
    if (!parsed) return '—';
    return parsed.toLocaleString('pt-BR', { timeZone: 'America/Campo_Grande', day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' });
  }
  ...
  function activeDays(rows) { return new Set(rows.filter((row) => Number(row.heartbeatCount || 0) > 0).map((row) => row.usageDate)).size; }
  ...
  function renderDoctors() {
    ...
    listEl.innerHTML = doctors.map((doctor) => {
      const guide30 = sum(rowsSince(doctor, 30), 'guideVisits');
      ...
      return `<button class="usage-doctor ${state.selected === doctor.username ? 'active' : ''}" type="button" data-doctor="${escapeHtml(doctor.username)}">
        <span class="usage-doctor-main"><strong>${escapeHtml(doctor.name || doctor.username)}</strong><small>${escapeHtml(doctor.jobTitle || `@${doctor.username}`)}</small><span class="usage-doctor-presence ${doctor.online ? 'online' : ''}"><span class="usage-presence-dot"></span>${escapeHtml(presence)}</span></span>
  ...
  function weekLabel(startKey) {
    ...
    return `${start.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', timeZone: 'UTC' })} – ${end.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric', timeZone: 'UTC' })}`;
  }
  ...
  function aggregateHistory(rows, period) {
    ...
    rows.forEach((row) => {
      const key = period === 'month' ? row.usageDate.slice(0, 7) : period === 'week' ? weekStartKey(row.usageDate) : row.usageDate;
      if (!groups.has(key)) groups.set(key, { key, rows: [], visits: 0, guideVisits: 0, firstSeen: null, lastSeen: null });
  ...
  function renderHistory(doctor) {
    ...
    historyBody.innerHTML = groups.map((group) => `<tr><td><strong>${escapeHtml(group.label)}</strong></td><td>${group.activeDays}</td><td>${group.guideVisits || '<span class="zero">0</span>'}</td><td>${group.visits || '<span class="zero">0</span>'}</td><td>${state.period === 'day' ? escapeHtml(formatClock(group.firstSeen)) : escapeHtml(formatDateTime(group.firstSeen))}</td><td>${state.period === 'day' ? escapeHtml(formatClock(group.lastSeen)) : escapeHtml(formatDateTime(group.lastSeen))}</td></tr>`).join('');
  }

  function selectedDoctor() { return state.doctors.find((doctor) => doctor.username === state.selected) || null; }

  function renderSelected() {
    const doctor = selectedDoctor();
    const empty = document.getElementById('usageEmptyState');
    const selected = document.getElementById('usageSelected');
    if (!doctor) { empty.hidden = false; selected.hidden = true; return; }
    empty.hidden = true;
  ...
  function renderAll() { renderSummary(); renderDoctors(); renderSelected(); }

  async function loadUsage({ quiet = false } = {}) {
    if (!quiet) listEl.innerHTML = '<div class="portal-note info">Carregando profissionais...</div>';
    try {
      const response = await fetch(`${endpoint}/api/admin/usage?days=370`, { method: 'GET', cache: 'no-store', headers: { 'Content-Type': 'application/json', ...auth.authorizationHeader() } });
      ...
      if (!state.selected && state.doctors.length) state.selected = (state.doctors.find((doctor) => doctor.online) || state.doctors[0]).username;

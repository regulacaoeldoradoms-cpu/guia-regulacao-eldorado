...
(() => {
  ...
  const PRINT_BUTTON_ID = 'printCompleteOrientation';
  const LEGACY_MISSING_BUTTON_ID = 'printMissingItems';
  ...
  function loadProtocols() {
    if (!protocolsPromise) {
      protocolsPromise = fetch(`${DATA_SOURCE}?patient-orientation=6.0`, { cache: 'no-store' })
        .then((response) => {
  ...
  function sectionHtml(title, items, note = '', className = '') {
    ...
    return `<section class="section ${className}"><h2>${escapeHtml(title)}</h2>${note ? `<p class="section-note">${escapeHtml(note)}</p>` : ''}<ul>${items.map((item) => `<li>${escapeHtml(item)}</li>`).join('')}</ul></section>`;
  }

  function printableHtml(protocol, subprotocol) {
    const official = officialData(protocol, subprotocol);
    const today = new Intl.DateTimeFormat('pt-BR').format(new Date());
    const heading = 'ORIENTAÇÃO PARA SOLICITAÇÃO';
    ...
    return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(heading)}</title><style>
      @page{size:A4;margin:13mm}*{box-sizing:border-box}body{margin:0;color:#173247;font-family:Arial,Helvetica,sans-serif;background:#fff}.doc{max-width:790px;margin:0 auto}.header{display:flex;align-items:center;gap:15px;padding-bottom:13px;border-bottom:3px solid #0f7881}.header img{width:68px;height:68px;border-radius:14px;object-fit:contain}.header h1{margin:0 0 5px;color:#0d3157;font-size:19px;line-height:1.2}.header p{margin:0;color:#537087;font-size:12px}.print-help{margin:0 0 14px;padding:10px 12px;border:1px solid #ead89b;border-radius:8px;background:#fff8df;color:#5f4b12;font-size:12px}.meta{margin:17px 0 14px;padding:13px 15px;border:1px solid #d5e3ed;border-radius:11px;background:#f7fbfd}.meta p{margin:4px 0;font-size:12px;line-height:1.45}.intro{margin:14px 0 18px;padding:13px 14px;border-left:4px solid #0f7881;background:#f2fbfb;border-radius:8px;font-size:12px;line-height:1.55}.section{break-inside:avoid;margin:18px 0 0}.section h2{margin:0 0 7px;color:#0d3157;font-size:15px}.section-note{margin:0 0 7px;color:#657c8d;font-size:11px;line-height:1.45}ul{margin:0;padding-left:21px}li{margin:6px 0;font-size:12.4px;line-height:1.48}.mandatory{padding:11px 13px;border-left:3px solid #0d3157;background:#f7f9fb}.conditional{padding:11px 13px;border-left:3px solid #9b7425;background:#fffaf0}.optional{padding:11px 13px;border-left:3px solid #6c8797;background:#f8fafb}.footer{margin-top:25px;padding-top:11px;border-top:1px solid #bccbd6;color:#536a7c;font-size:10.3px;line-height:1.5}@media print{body{print-color-adjust:exact;-webkit-print-color-adjust:exact}.print-help{display:none!important}}
      ...
      <section class="meta"><p><strong>Especialidade/exame:</strong> ${escapeHtml(displayName(protocol))}</p>${subprotocol?.titulo ? `<p><strong>Motivo/condição:</strong> ${escapeHtml(subprotocol.titulo)}</p>` : ''}${protocol.faixaEtaria ? `<p><strong>Faixa etária do protocolo:</strong> ${escapeHtml(protocol.faixaEtaria)}</p>` : ''}${protocol.solicitante ? `<p><strong>Profissional solicitante:</strong> ${escapeHtml(protocol.solicitante)}</p>` : ''}<p><strong>Data da orientação:</strong> ${today}</p></section>
      ...
      <footer class="footer"><strong>Setor de Regulação de Saúde</strong><br>Bairro Jardim das Grevílias, Rua Irmã Aristela, nº 836 · Eldorado/MS<br>Atendimento: segunda a sexta-feira, das 07:00 às 11:00.<br><br><strong>Fonte:</strong> ${escapeHtml(sources)}<br>Esta orientação auxilia a preparação da solicitação e não representa autorização de consulta, exame ou procedimento.</footer>
    </main></body></html>`;
  }
  ...
  function printHtml(html) {
    ...
    if (popup) {
      ...
      waitForImages(popup.document, () => { if (!popup.closed) { popup.focus(); try { popup.print(); } catch (_) {} } });
      return;
    }
    ...
    waitForImages(doc, () => { try { frame.contentWindow.focus(); frame.contentWindow.print(); } finally { window.setTimeout(() => frame.remove(), 2500); } });
  }
  ...
  async function handlePrint() {
    try {
      ...
      printHtml(printableHtml(protocol, subprotocol));
    } catch (error) {
      showError(error.message || 'Não foi possível montar a orientação para esta solicitação.');
    }
  }

  function cleanLegacyChecklistPrint(detail) {
    detail.querySelector(`#${LEGACY_MISSING_BUTTON_ID}`)?.remove();
    const summaryText = detail.querySelector('#receptionSummaryText');
    if (summaryText && /imprima somente o que precisa ser providenciado/i.test(summaryText.textContent || '')) {
      summaryText.textContent = (summaryText.textContent || '').replace(/\s*Imprima somente o que precisa ser providenciado\.?/i, ' O checklist é somente para acompanhamento interno da recepção.');
    }
  }
  ...
  function ensurePrintButton() {
    ...
    if (!detail || !detail.querySelector('.reception-title')) return;
    cleanLegacyChecklistPrint(detail);
    if (detail.querySelector(`#${PRINT_BUTTON_ID}`)) return;
    const box = document.createElement('div');
    box.className = 'portal-note info reception-complete-orientation';
    box.innerHTML = '<strong>Orientação para o paciente</strong><br><span>A impressão mostra os requisitos do protocolo oficial. As marcações do checklist servem somente para a conferência interna da recepção e não alteram o documento.</span><br><br><button class="portal-button primary" id="printCompleteOrientation" type="button">Imprimir orientação</button>';
  ...
  function scheduleEnsure() {
    if (queued) return;
    queued = true;
    requestAnimationFrame(ensurePrintButton);
  }
  ...
  const detail = document.getElementById(DETAIL_ID);
  if (detail) {
    new MutationObserver(scheduleEnsure).observe(detail, { childList: true, subtree: true, characterData: true });
    detail.addEventListener('change', scheduleEnsure, true);
    scheduleEnsure();
  }
})();

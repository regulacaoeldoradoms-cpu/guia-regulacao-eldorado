...
function isDermatologyProtocol(protocol) {
  const text = normalizeDermatologyStatus(`${protocol?.id || ''} ${protocol?.nome || ''} ${(protocol?.tags || []).join(' ')}`);
...
function applyDermatologyTeleconsultStatus() {
  ...
  protocol.fontes = [...new Set([...(Array.isArray(protocol.fontes) ? protocol.fontes : []), 'Atualização operacional do Setor de Regulação de Eldorado/MS — 28/07/2026'])];

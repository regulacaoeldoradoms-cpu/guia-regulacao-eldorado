...
function institutionalArray(value) {
  return Array.isArray(value) ? value : (value ? [value] : []);
}

function institutionalEscape(value) {
...
function institutionalList(items) {
  ...
  return `<ul>${values.map((item) => `<li>${institutionalEscape(item)}</li>`).join('')}</ul>`;
}

function institutionalSection(title, items, className = '') {
  const values = institutionalArray(items);
  if (!values.length) return '';
  return `<section class="institutional-section ${className}"><h2>${institutionalEscape(title)}</h2>${institutionalList(values)}</section>`;
}

function institutionalSubprotocols(protocol) {
  return institutionalArray(protocol.subprotocolos).map((subprotocol) => {
    const content = [
    ...
    if (!content) return '';
    return `<section class="institutional-subprotocol"><h2>${institutionalEscape(subprotocol.titulo || 'Condição específica')}</h2>${content}</section>`;
  }).join('');
}
...
function printInstitutionalOrientation(protocol) {
  ...
  printArea.innerHTML = `
    <article class="institutional-document">
      ...
      <footer class="institutional-footer">
        ...
        <div class="institutional-validation">
          ...
          <span>Fonte: ${institutionalEscape(institutionalArray(protocol.fontes).join(' · ') || 'protocolo vigente')} · Última conferência: ${institutionalEscape(protocol.ultimaConferencia || '28/07/2026')}</span>
        </div>
      </footer>
    </article>`;
...
// O listener em captura impede que a impressão técnica antiga seja executada.

...
(() => {
  ...
  function levelFor(user) {
    return normalizeLevel(user?.accountLevel || user?.accountProgress?.level || (user?.emailVerified ? 'prata' : 'bronze'));
  }
  ...
  function renderProgress(root, user) {
    ...
    if (achievement) {
      achievement.innerHTML = current.level === 'bronze'
        ...
        : current.level === 'prata'
          ? '<span class="achievement-icon">🎉</span><div><strong>Foto de perfil desbloqueada</strong><span>Sua conta verificada já pode usar foto e está preparada para recursos sociais futuros.</span></div>'

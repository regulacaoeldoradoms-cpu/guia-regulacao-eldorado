...
- Estado neste documento: **incidente recuperado e homologado em produção em 18/09/2026**. A Agenda voltou a listar os registros anteriores e a sincronização autorizada foi confirmada pelo operador após a recuperação.
...
- Recuperação concluída em 18/09: `AGENDA_RECUPERADA`, nova versão do Worker promovida após validação de credencial, 6 permissões Firestore, `AUTH_DB` e preservação dos secrets existentes. O probe anônimo retornou 403, comprovando que a rota voltou a ultrapassar o guard de armazenamento e alcançou a autenticação. Em seguida o operador abriu `/agenda/`, confirmou o reaparecimento dos dados anteriores e confirmou funcionamento real da Agenda.
- Prevenção permanente aprovada: substituir o deploy monolítico do Worker por `worker/scripts/deploy-safe.mjs`, documentado em `docs/WORKER-SAFE-DEPLOY.md`. O gate envia a candidata sem tráfego, valida bindings críticos, todos os secrets atuais e o mesmo `AUTH_DB`, promove somente após validação, testa `/api/agenda` e executa rollback automático se o pós-deploy falhar.
- Ajuste preventivo em 18/09/2026: uma candidata sem tráfego criada pelo próprio gate (`dfc7fe8f-182b-4a95-8054-6333e13291d8`) permaneceu como Worker Version mais recente e o bloqueio antigo `ULTIMA_VERSAO_NAO_E_A_PRODUCAO_PARE_E_REVISE` impediu novos builds, embora a produção continuasse íntegra em 100%. A política foi refinada para não depender de exclusão manual de versões: somente candidatas identificadas pela mensagem/tag reservada do gate podem ser tratadas como órfãs recuperáveis, e ainda assim precisam preservar bindings críticos, todos os nomes/tipos de secrets, Firebase estável e o mesmo `AUTH_DB` da produção. O upload subsequente usa `secrets.required` dinâmico; versões de origem desconhecida ou divergentes continuam bloqueadas. O modo global `--strict` foi removido porque conflita com bindings legítimos gerenciados remotamente sob `keep_vars=true`; a candidata continua sendo inspecionada integralmente antes da promoção.
- O log completo do Workers Builds de 18/09/2026 mostrou depois um bloqueio ainda na etapa 1: `DEPLOY_SEGURO_INTERROMPIDO=BINDING_CRITICO_AUSENTE_GEMINI_API_KEY`, com 330/330 testes aprovados antes do gate. Isso comprovou que a baseline produtiva atual já não expõe `GEMINI_API_KEY` como binding. Como o objetivo do gate é impedir regressão de configuração, não impor uma configuração nova à produção, `GEMINI_API_KEY` deixou de ser requisito fixo: quando existir em produção, continua obrigatoriamente preservada pela regra dinâmica de secrets; quando já estiver ausente, a ausência pré-existente não bloqueia todos os deploys. A disponibilidade do Gemini permanece uma verificação operacional separada no diagnóstico do Portal.
...
Essas quatro etapas foram concluídas pelo operador em 18/09/2026. O incidente deixa de ser uma pendência operacional e passa a ser referência de prevenção de regressão.

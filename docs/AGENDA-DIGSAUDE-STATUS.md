...
- Em 18/09 foi preparada a recuperação controlada `scripts/agenda/recuperar-firebase-agenda.mjs`. Ela não recebe nem imprime valores secretos. O fluxo confirma o 503, inspeciona nomes/tipos dos bindings, localiza uma versão da Agenda previamente homologada, gera uma nova versão da `main` sem tráfego com herança explícita de segredos por `version_id`, valida todos os bindings e só então promove a versão validada.
